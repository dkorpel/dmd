// Shared catalog of D snippets, the single source of truth for both:
//   * the tour's inline interactive demos (tour.html) — a <div class="demo">
//     references one or more entries by id via data-examples="id1,id2,…", and
//   * the explorer's "Load an example…" dropdown (index.html), which lists every
//     entry here.
//
// To add a demo: append one entry below and (optionally) reference its id from a
// tour demo. It shows up in the explorer dropdown automatically — no code is ever
// copy-pasted between the two pages.
//
// Fields: id (stable key), label (dropdown / demo-select text), panes (comma list
// of explorer panes to show: lex, parse, sema, ast, ir, iropt, asm, diag), height
// (iframe px in the tour), code (the D source).
...
`,
  },
  {
    ...
    panes: "parse,diag",
    height: 320,
    code: `// Whether something is a parse error or semantic error can be arbitrary

void foo()()
{
	// Redundant attributes are allowed per grammar,
    // but the parser raises an error
	shared const shared int x = 0;

	// Cases outside a switch could be disallowed by grammar,
