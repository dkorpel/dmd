...
const wasi = new Proxy({
  fd_write: (fd, iovs, n, nw) => { const v=dv(); let w=0,s="";
    for(let i=0;i<n;i++){const p=v.getUint32(iovs+i*8,true),l=v.getUint32(iovs+i*8+4,true);
      s+=td.decode(new Uint8Array(memory.buffer,p,l));w+=l;} stderrText+=s; v.setUint32(nw,w,true); return 0; },
...
function compile(src){ stderrText="";
  ...
  return {ast, errors:ex.dmdwasm_errors(), diag:stderrText}; }
