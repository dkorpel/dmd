...
//                      p.adFlags |= AST.VarDeclaration.nounderscore;
            ...
            switch (token.value)
            {
                case TOK.identifier:
                    if (s)
                    {
                        eSink.error(token.loc, "missing comma or semicolon after declaration of `%s`, found `%s` instead", s.toChars(), token.toChars());
                        goto Lend;
                    }
                    goto default;
    ...
    AST.Dsymbol cparseFunctionDefinition(Identifier id, AST.TypeFunction ft, ref Specifier specifier)
    {
        ...
        if (token.value != TOK.leftCurly)       // if not start of a compound-statement
        {
            ...
            if (symbols.length != plLength)
                eSink.error(token.loc, "%d identifiers does not match %d declarations", cast(int)plLength, cast(int)symbols.length);
    ...
    private void pragmaPack(const ref Loc startloc, bool useScan)
    {
        ...
        void setPackAlign(ref const Token t)
        {
            ...
            if (n < 1 || n & (n - 1) || ushort.max < n)
                eSink.error(loc, "pack must be an integer positive power of 2, not 0x%llx", cast(ulong)n);
