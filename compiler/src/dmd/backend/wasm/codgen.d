...
enum : ubyte
{
    ...
    OP_I64_GE_U    = 0x5A,
    // f32/f64 comparisons
    OP_F32_EQ = 0x5B, OP_F32_NE = 0x5C, OP_F32_LT = 0x5D, OP_F32_GT = 0x5E,
    OP_F32_LE = 0x5F, OP_F32_GE = 0x60,
    OP_F64_EQ = 0x61, OP_F64_NE = 0x62, OP_F64_LT = 0x63, OP_F64_GT = 0x64,
    OP_F64_LE = 0x65, OP_F64_GE = 0x66,
    // i32 arithmetic
    OP_I32_CLZ     = 0x67,
    OP_I32_CTZ     = 0x68,
    OP_I32_ADD     = 0x6A,
...
// Value type bytes
enum : ubyte { WASM_I32=0x7F, WASM_I64=0x7E, WASM_F32=0x7D, WASM_F64=0x7C }
...
{
    do { ubyte bt = v & 0x7F; v >>= 7; if (v) bt |= 0x80; b.writeByte(bt); } while (v);
}
...
ubyte wasmType(tym_t ty) @trusted
{
    switch (tybasic(ty))
    {
        case TYllong: case TYullong: case TYcent: case TYucent:
            return WASM_I64;
...
nothrow:
    ...
    void emit(ubyte b) @trusted { code.writeByte(b); }

    void emitULEB(uint v) @trusted { uleb(&code, v); }

    void emitSLEB(long v) @trusted { sleb(&code, v); }
...
private bool genElem(ref WasmCG cg, elem* e) @trusted
{
    ...
    case OPconst:
    {
        ...
        {
        case TYllong: case TYullong: case TYcent: case TYucent:
            cg.emit(OP_I64_CONST); cg.emitSLEB(e.Vllong); break;
        case TYfloat: case TYifloat:
        {
        ...
        default:
            cg.emit(OP_I32_CONST); cg.emitSLEB(cast(int) e.Vlong); break;
        }
        return true;
    }
    ...
    case OPvar:
    {
        ...
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);
        return true;
    }
    ...
    case OPrelconst:
    {
        ...
        cg.emit(OP_I32_CONST); cg.emitSLEB(cast(int) e.Voffset);
        return true;
    }

    // ---- Indirection (load from memory) -----------------------------------
    case OPind:
    {
        ...
        case TYllong: case TYullong:
            cg.emit(OP_I64_LOAD); cg.emitMemArg(3, 0); break;
        case TYfloat:
            cg.emit(OP_F32_LOAD); cg.emitMemArg(2, 0); break;
        case TYdouble: case TYdouble_alias: case TYreal:
            cg.emit(OP_F64_LOAD); cg.emitMemArg(3, 0); break;
        case TYchar: case TYschar:
            cg.emit(OP_I32_LOAD8_S); cg.emitMemArg(0, 0); break;
        case TYuchar: case TYbool:
            cg.emit(OP_I32_LOAD8_U); cg.emitMemArg(0, 0); break;
        case TYshort:
            cg.emit(OP_I32_LOAD16_S); cg.emitMemArg(1, 0); break;
        case TYwchar_t: case TYushort:
            cg.emit(OP_I32_LOAD16_U); cg.emitMemArg(1, 0); break;
        default:
            cg.emit(OP_I32_LOAD); cg.emitMemArg(2, 0); break;
        }
        return true;
    }
    ...
    case OPeq:
    {
        if (e.E1.Eoper == OPvar)
        {
            // Simple local assignment: value → local.set
            genElem(cg, e.E2);
            const uint idx = cg.localFor(e.E1.Vsym);
            cg.emit(OP_LOCAL_TEE); cg.emitULEB(idx);  // tee leaves copy on stack
            return true;
        }
        else if (e.E1.Eoper == OPind)
        {
            ...
            {
            case TYllong: case TYullong:
                cg.emit(OP_I64_STORE); cg.emitMemArg(3, 0); break;
            case TYfloat:
                cg.emit(OP_F32_STORE); cg.emitMemArg(2, 0); break;
            case TYdouble: case TYdouble_alias: case TYreal:
                cg.emit(OP_F64_STORE); cg.emitMemArg(3, 0); break;
            case TYchar: case TYschar: case TYuchar: case TYbool:
                cg.emit(OP_I32_STORE8); cg.emitMemArg(0, 0); break;
            case TYshort: case TYwchar_t: case TYushort:
                cg.emit(OP_I32_STORE16); cg.emitMemArg(1, 0); break;
            default:
                cg.emit(OP_I32_STORE); cg.emitMemArg(2, 0); break;
            }
    ...
    {
        const ty = tybasic(e.Ety);
        if (ty == TYfloat) { genElem(cg, e.E1); cg.emit(OP_F32_NEG); }
        else if (ty == TYdouble || ty == TYdouble_alias || ty == TYreal)
            { genElem(cg, e.E1); cg.emit(OP_F64_NEG); }
        else if (ty == TYllong || ty == TYullong)
        {
            // i64.neg = 0 - x
            cg.emit(OP_I64_CONST); cg.emitSLEB(0);
            genElem(cg, e.E1);
            cg.emit(OP_I64_SUB);
        }
        else
    ...
    case OPnot:
    {
        genElem(cg, e.E1);
        const ty = tybasic(e.E1.Ety);
        if (ty == TYllong || ty == TYullong) cg.emit(OP_I64_EQZ);
        else cg.emit(OP_I32_EQZ);
        return true;
    }
    ...
    case OPcom:
    {
        ...
        {
            cg.emit(OP_I64_CONST); cg.emitSLEB(-1);
            cg.emit(OP_I64_XOR);
        }
        else
        {
    ...
    // ---- Type conversions ------------------------------------------------
    case OPu16_32: case OPs16_32:
    case OPu8_16:  case OPs8_16:
    {
        genElem(cg, e.E1);
        if (op == OPs8_16)             { cg.emit(OP_I32_EXTEND8_S); }
        else if (op == OPs16_32)       { cg.emit(OP_I32_EXTEND16_S); }
        // unsigned widening: already zero-extended as i32 — no-op
        return true;
    }

    case OPu32_64: { genElem(cg, e.E1); cg.emit(OP_I64_EXTEND_I32_U); return true; }
    case OPs32_64: { genElem(cg, e.E1); cg.emit(OP_I64_EXTEND_I32_S); return true; }
    case OP64_32:  { genElem(cg, e.E1); cg.emit(OP_I32_WRAP_I64); return true; }

    case OPd_f:    { genElem(cg, e.E1); cg.emit(OP_F32_DEMOTE_F64);    return true; }
    case OPf_d:    { genElem(cg, e.E1); cg.emit(OP_F64_PROMOTE_F32);   return true; }

    case OPd_s32:  { genElem(cg, e.E1); cg.emit(OP_I32_TRUNC_F64_S); return true; }
    case OPd_s64:  { genElem(cg, e.E1); cg.emit(OP_I64_TRUNC_F64_S); return true; }
    case OPs32_d:  { genElem(cg, e.E1); cg.emit(OP_F64_CONVERT_I32_S); return true; }
    case OPs64_d:  { genElem(cg, e.E1); cg.emit(OP_F64_CONVERT_I64_S); return true; }
    case OPu32_d:  { genElem(cg, e.E1); cg.emit(OP_F64_CONVERT_I32_U); return true; }
    case OPu64_d:  { genElem(cg, e.E1); cg.emit(OP_F64_CONVERT_I64_S); return true; }

    // ---- Comma operator (sequence) ----------------------------------------
    case OPcomma:
    {
        const bool r1 = genElem(cg, e.E1);
        if (r1) cg.emit(OP_DROP);    // discard left-hand result
    ...
    case OPoror:
    {
        // a || b  →  if (a) 1 else (b != 0)
        genElem(cg, e.E1);
        cg.emit(OP_IF); cg.emit(WASM_I32);
        cg.emit(OP_I32_CONST); cg.emitSLEB(1);
        cg.emit(OP_ELSE);
        genElem(cg, e.E2);
        cg.emit(OP_I32_CONST); cg.emitSLEB(0);
    ...
    case OPpostinc: case OPpostdec:
    {
        ...
        {
            const uint idx = cg.localFor(e.E1.Vsym);
            cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);  // old value (result)
            cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);
            genElem(cg, e.E2);
            emitBinop(cg, op == OPpostinc ? OPadd : OPmin, e.Ety);
            cg.emit(OP_LOCAL_SET); cg.emitULEB(idx);
            return true;
        }
        genElem(cg, e.E1);
        return true;
    ...
    // ---- Pair (multiple return) — emit both halves -----------------------
...
private void emitBinop(ref WasmCG cg, int op, tym_t ty) @trusted
{
    ...
    {
    case OPadd:
        if (isF32) cg.emit(OP_F32_ADD);
        else if (isF64) cg.emit(OP_F64_ADD);
        else if (is64) cg.emit(OP_I64_ADD);
        else cg.emit(OP_I32_ADD);
        break;
    case OPmin:
        if (isF32) cg.emit(OP_F32_SUB);
        else if (isF64) cg.emit(OP_F64_SUB);
        else if (is64) cg.emit(OP_I64_SUB);
        else cg.emit(OP_I32_SUB);
        break;
    case OPmul:
        if (isF32) cg.emit(OP_F32_MUL);
        else if (isF64) cg.emit(OP_F64_MUL);
        else if (is64) cg.emit(OP_I64_MUL);
        else cg.emit(OP_I32_MUL);
        break;
    case OPdiv:
        if (isF32) cg.emit(OP_F32_DIV);
        else if (isF64) cg.emit(OP_F64_DIV);
        else if (is64) cg.emit(isUns ? OP_I64_DIV_U : OP_I64_DIV_S);
        else cg.emit(isUns ? OP_I32_DIV_U : OP_I32_DIV_S);
        break;
    case OPmod:
        if (is64) cg.emit(isUns ? OP_I64_REM_U : OP_I64_REM_S);
        else cg.emit(isUns ? OP_I32_REM_U : OP_I32_REM_S);
        break;
    case OPand:
        cg.emit(is64 ? OP_I64_AND : OP_I32_AND); break;
    case OPor:
        cg.emit(is64 ? OP_I64_OR  : OP_I32_OR);  break;
    case OPxor:
        cg.emit(is64 ? OP_I64_XOR : OP_I32_XOR); break;
    case OPshl:
        cg.emit(is64 ? OP_I64_SHL : OP_I32_SHL); break;
    case OPshr:
        cg.emit(is64 ? OP_I64_SHR_U : OP_I32_SHR_U); break;
    case OPashr:
        cg.emit(is64 ? OP_I64_SHR_S : OP_I32_SHR_S); break;
    default:
        cg.emit(OP_UNREACHABLE); break;
    }
}
...
private int compoundToBinop(int op) @safe
{
    switch (op)
    {
    case OPaddass:  return OPadd;
    case OPminass:  return OPmin;
    case OPmulass:  return OPmul;
    case OPdivass:  return OPdiv;
    case OPmodass:  return OPmod;
    case OPandass:  return OPand;
    case OPorass:   return OPor;
    case OPxorass:  return OPxor;
    case OPshlass:  return OPshl;
    case OPshrass:  return OPshr;
    case OPashrass: return OPashr;
...
private void emitRelop(ref WasmCG cg, int op, tym_t operandTy) @trusted
{
    ...
    const bool isF64 = (tybasic(operandTy) == TYdouble || tybasic(operandTy) == TYdouble_alias || tybasic(operandTy) == TYreal);
    ...
    if (isF32)
    {
        switch (op)
        {
        case OPeqeq: cg.emit(OP_F32_EQ); break;
        case OPne:   cg.emit(OP_F32_NE); break;
        case OPlt:   cg.emit(OP_F32_LT); break;
        case OPle:   cg.emit(OP_F32_LE); break;
        case OPgt:   cg.emit(OP_F32_GT); break;
        case OPge:   cg.emit(OP_F32_GE); break;
        default:     cg.emit(OP_UNREACHABLE); break;
        }
        return;
    }
    if (isF64)
    {
        switch (op)
        {
        case OPeqeq: cg.emit(OP_F64_EQ); break;
        case OPne:   cg.emit(OP_F64_NE); break;
        case OPlt:   cg.emit(OP_F64_LT); break;
        case OPle:   cg.emit(OP_F64_LE); break;
        case OPgt:   cg.emit(OP_F64_GT); break;
        case OPge:   cg.emit(OP_F64_GE); break;
        default:     cg.emit(OP_UNREACHABLE); break;
        }
        return;
    }
    if (is64)
    {
        switch (op)
        {
        case OPeqeq: cg.emit(OP_I64_EQ); break;
        case OPne:   cg.emit(OP_I64_NE); break;
        case OPlt:   cg.emit(isUns ? OP_I64_LT_U : OP_I64_LT_S); break;
        case OPle:   cg.emit(isUns ? OP_I64_LE_U : OP_I64_LE_S); break;
        case OPgt:   cg.emit(isUns ? OP_I64_GT_U : OP_I64_GT_S); break;
        case OPge:   cg.emit(isUns ? OP_I64_GE_U : OP_I64_GE_S); break;
        default:     cg.emit(OP_UNREACHABLE); break;
        }
        return;
    }
    switch (op)
    {
    case OPeqeq: cg.emit(OP_I32_EQ); break;
    case OPne:   cg.emit(OP_I32_NE); break;
    case OPlt:   cg.emit(isUns ? OP_I32_LT_U : OP_I32_LT_S); break;
    case OPle:   cg.emit(isUns ? OP_I32_LE_U : OP_I32_LE_S); break;
    case OPgt:   cg.emit(isUns ? OP_I32_GT_U : OP_I32_GT_S); break;
    case OPge:   cg.emit(isUns ? OP_I32_GE_U : OP_I32_GE_S); break;
    default:     cg.emit(OP_UNREACHABLE); break;
    }
}
...
private void genBlocksProper(ref WasmCG cg, block* startblock, bool hasReturn) @trusted
{
    ...
    struct Frame { bool isLoop; int closeAfter; }
    Frame[] stack;
    int depth() { return cast(int) stack.length; }

    // br depth to reach a given stack frame (0 = innermost)
    uint brDepth(size_t frameIdx) { return cast(uint)(stack.length - 1 - frameIdx); }
    ...
    for (size_t bi = 0; bi < N; bi++)
    {
        ...
        {
            if (b.Belem) { const bool v = genElem(cg, b.Belem); if (v) cg.emit(OP_DROP); }
            cg.emit(OP_RETURN);
            continue;
        }
...
// ---------------------------------------------------------------------------
// Main entry point called from dout.d
// ---------------------------------------------------------------------------
