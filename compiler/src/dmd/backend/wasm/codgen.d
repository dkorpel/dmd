...
// Synthesize a built-in function body (memcpy/memset/memcmp) if not already done.
// Returns the function index, or uint.max if not a known built-in.
private uint synthBuiltin(const(char)* name) @trusted
{
    import core.stdc.string : strcmp;
    import dmd.backend.wasmobj : wasmFuncBodies, WasmFuncBody, WasmLocal,
        wmod_numImports, wmod_addDefinedFunc;

    // Check if already synthesized.
    uint nimports = wmod_numImports();
    foreach (size_t i, ref const WasmFuncBody fb; wasmFuncBodies)
        if (fb.sym == null && fb.name == name[0 .. strlen(name)])
            return nimports + cast(uint) i;

    WasmFuncBody fb;
    fb.sym = null;
    fb.name = name[0 .. strlen(name)].idup;
    fb.numParams = 0;

    WasmCG cg;

    if (strcmp(name, "memcpy") == 0 || strcmp(name, "_memcpy") == 0)
    {
        // memcpy(dst: i32, src: i32, count: i32) → i32 (returns dst)
        WasmLocal p0; p0.ty = WASM_I32; cg.locals ~= p0; // dst
        WasmLocal p1; p1.ty = WASM_I32; cg.locals ~= p1; // src
        WasmLocal p2; p2.ty = WASM_I32; cg.locals ~= p2; // count
        cg.numParams = 3;
        uint idx = cg.allocTemp(WASM_I32); // i
        // i = 0; while i < count: dst[i] = src[i]; i++
        cg.emit(OP_I32_CONST); cg.emitSLEB(0);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(idx);
        cg.emit(OP_BLOCK); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_LOOP); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(2); // count
        cg.emit(OP_I32_GE_U); cg.emit(OP_BR_IF); cg.emitULEB(1);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(0); // dst
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx); cg.emit(OP_I32_ADD);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(1); // src
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx); cg.emit(OP_I32_ADD);
        ...
        cg.emit(OP_BR); cg.emitULEB(0);
        cg.emit(OP_END); cg.emit(OP_END);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(0); // return dst
        cg.emit(OP_RETURN);
        {
    ...
    if (strcmp(name, "memset") == 0 || strcmp(name, "_memset") == 0)
    {
        // memset(dst: i32, val: i32, count: i32) → i32 (returns dst)
        WasmLocal p0; p0.ty = WASM_I32; cg.locals ~= p0; // dst
        WasmLocal p1; p1.ty = WASM_I32; cg.locals ~= p1; // val
        WasmLocal p2; p2.ty = WASM_I32; cg.locals ~= p2; // count
        cg.numParams = 3;
        uint idx = cg.allocTemp(WASM_I32);
        cg.emit(OP_I32_CONST); cg.emitSLEB(0);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(idx);
        cg.emit(OP_BLOCK); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_LOOP); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(2);
        cg.emit(OP_I32_GE_U); cg.emit(OP_BR_IF); cg.emitULEB(1);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(0);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx); cg.emit(OP_I32_ADD);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(1);
        cg.emit(OP_I32_STORE8); cg.emitMemArg(0, 0);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx);
        cg.emit(OP_I32_CONST); cg.emitSLEB(1); cg.emit(OP_I32_ADD);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(idx);
    ...
    if (strcmp(name, "memcmp") == 0 || strcmp(name, "_memcmp") == 0)
    {
        // memcmp(a: i32, b: i32, count: i32) → i32
        WasmLocal p0; p0.ty = WASM_I32; cg.locals ~= p0;
        WasmLocal p1; p1.ty = WASM_I32; cg.locals ~= p1;
        WasmLocal p2; p2.ty = WASM_I32; cg.locals ~= p2;
        cg.numParams = 3;
        uint idx = cg.allocTemp(WASM_I32);
        uint va  = cg.allocTemp(WASM_I32);
        uint vb  = cg.allocTemp(WASM_I32);
        cg.emit(OP_I32_CONST); cg.emitSLEB(0);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(idx);
        cg.emit(OP_BLOCK); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_LOOP); cg.emit(WASM_VOID_BLOCK);
        ...
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx); cg.emit(OP_I32_ADD);
        cg.emit(OP_I32_LOAD8_U); cg.emitMemArg(0, 0);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(va);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(1);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(idx); cg.emit(OP_I32_ADD);
        cg.emit(OP_I32_LOAD8_U); cg.emitMemArg(0, 0);
        cg.emit(OP_LOCAL_SET); cg.emitULEB(vb);
        // if va < vb: return -1
        cg.emit(OP_LOCAL_GET); cg.emitULEB(va);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(vb);
        cg.emit(OP_I32_LT_U);
        cg.emit(OP_IF); cg.emit(WASM_VOID_BLOCK);
        cg.emit(OP_I32_CONST); cg.emitSLEB(-1); cg.emit(OP_RETURN);
        cg.emit(OP_END);
        // if va > vb: return 1
        cg.emit(OP_LOCAL_GET); cg.emitULEB(va);
        cg.emit(OP_LOCAL_GET); cg.emitULEB(vb);
        ...
        cg.emit(OP_BR); cg.emitULEB(0);
        cg.emit(OP_END); cg.emit(OP_END);
        cg.emit(OP_I32_CONST); cg.emitSLEB(0); // equal
        cg.emit(OP_RETURN);
        {
            uint idx2 = wmod_addDefinedFunc(fb.name, cg.locals, cg.numParams,
                [WASM_I32, WASM_I32, WASM_I32], [WASM_I32]);
...
private uint funcIndex(Symbol* sfunc) @trusted
{
    ...
    // Check for known C runtime built-ins: synthesize inline instead of importing.
    if (sfunc && sfunc.Sident.ptr)
    {
        uint bfidx = synthBuiltin(sfunc.Sident.ptr);
        if (bfidx != uint.max)
            return bfidx;
    }
