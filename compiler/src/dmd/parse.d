...
class Parser(AST, Lexer = dmd.lexer.Lexer) : Lexer
{
  ...
  final:
    ...
    private bool parseDeprecatedAttribute(ref AST.Expression msg)
    {
        ...
        if (msg)
        {
            eSink.error(token.loc, "conflicting storage class `deprecated(%s)` and `deprecated(%s)`", msg.toChars(), e.toChars());
        }
        msg = e;
        return true;
    }
            ...
            case TOK.extern_:
                {
                    ...
                    if (pAttrs.link != LINK.default_)
                    {
                        ...
                        {
                            eSink.error(token.loc, "conflicting linkage `extern (%s)` and `extern (%s)`", AST.linkageToChars(pAttrs.link), AST.linkageToChars(res.link));
                        }
            ...
            Lprot:
                {
                    if (pAttrs.visibility.kind != AST.Visibility.Kind.undefined)
                    {
                        if (pAttrs.visibility.kind != prot)
                            eSink.error(token.loc, "conflicting visibility attribute `%s` and `%s`", AST.visibilityToChars(pAttrs.visibility.kind), AST.visibilityToChars(prot));
                        else
    ...
    private StorageClass appendStorageClass(StorageClass orig, StorageClass added)
    {
        ...
        if ((added & STC.in_) && (orig & Redundant))
        {
            ...
            else if (global.params.previewIn)
            {
                ...
                eSink.error(token.loc, "attribute `in` cannot be added after `%s`: remove `%s`",
                      stc_str, stc_str);
            }
            else
    ...
    private Identifier[] parseQualifiedIdentifier(const(char)* entity)
    {
        ...
        do
        {
            ...
            if (token.value != TOK.identifier)
            {
                eSink.error(token.loc, "`%s` expected as dot-separated identifiers, got `%s`", entity, token.toChars());
                return qualified;
            }
    ...
    private AST.Dsymbol parseAggregate()
    {
        ...
        if (token.value == TOK.leftCurly)
        {
            ...
            {
                /* { */
                eSink.error(token.loc, "`}` expected following members in `%s` declaration at %s",
    ...
    private AST.Type parseBasicType(bool dontLookDotIdents = false)
    {
        ...
        case TOK.mixin_:
            ...
            nextToken();
            if (token.value != TOK.leftParenthesis)
                eSink.error(token.loc, "found `%s` when expecting `%s` following `mixin`", token.toChars(), Token.toChars(TOK.leftParenthesis));
            auto exps = parseArguments();
    ...
    private AST.Dsymbols* parseDeclarations(bool autodecl, PrefixAttributes!AST* pAttrs, const(char)* comment)
    {
        ...
        while (1)
        {
            ...
            if (!tfirst)
                ...
                eSink.error(token.loc, "multiple declarations must have the same type, not `%s` and `%s`", tfirst.toChars(), t.toChars());
            ...
            if (isAliasDeclaration)
            {
                ...
                {
                    if (isThis)
                        eSink.error(token.loc, "cannot use syntax `alias this = %s`, use `alias %s this` instead", _init.toChars(), _init.toChars());
                    else
            ...
            else if (ident)
            {
                ...
                default:
                    ...
                    {
                        eSink.error(token.loc, "semicolon needed to end declaration of `%s`, instead of `%s`", s.toChars(), token.toChars());
    ...
    private void checkCstyleTypeSyntax(Loc loc, AST.Type t, int alt, Identifier ident)
    {
        ...
        eSink.error(loc, "instead of C-style syntax, use D-style `%s%s%s`", t.toChars(), sp, s);
    }
    ...
    private void closeCondition(string start, AST.Parameter param, AST.Expression condition)
    {
        ...
        if (format)
            eSink.error(token.loc, format.ptr, start.ptr, param ? "declaration".ptr : condition.toChars());
    }
...
LagainStc:
        ...
        Lexp:
            {
                ...
                {
                    eSink.error(token.loc, "found `%s` when expecting `;` or `=`, did you mean `%s %s = %s`?", peek(&token).toChars(), exp.toChars(), token.toChars(), peek(peek(&token)).toChars());
                    nextToken();
                }
                else
                {
    ...
    void check(Loc loc, TOK value)
    {
        if (token.value != value)
            eSink.error(loc, "found `%s` when expecting `%s`", token.toChars(), Token.toChars(value));
        nextToken();
    }
    ...
    {
        if (token.value != value)
            eSink.error(token.loc, "found `%s` when expecting `%s` following %s", token.toChars(), Token.toChars(value), string);
        nextToken();
    }
    ...
    private void checkParens(TOK value, AST.Expression e)
    {
        if (precedence[e.op] == PREC.rel && !e.parens)
            eSink.error(e.loc, "`%s` must be surrounded by parentheses when next to operator `%s`", e.toChars(), Token.toChars(value));
    }
    ...
    AST.Expression parsePrimaryExp()
    {
                    ...
                    if (token.value == TOK.string_)
                    {
                        if (token.postfix)
                        {
                            if (token.postfix != postfix)
                                eSink.error(token.loc, "mismatched string literal postfixes `'%c'` and `'%c'`", postfix, token.postfix);
        ...
        LabelX:
            ...
            if (token.value != TOK.identifier)
            {
                eSink.error(token.loc, "found `%s` when expecting identifier following `%s`.", token.toChars(), t.toChars());
                goto Lerr;
            }
        ...
        case TOK.is_:
            {
                ...
                if (token.value == TOK.leftParenthesis)
                {
                    ...
                    {
                        eSink.error(loc, "unexpected `(` after `%s`, inside `is` expression. Try enclosing the contents of `is` with a `typeof` expression", token.toChars());
                        nextToken();
